...
// Token Types
typedef enum {
    TOK_ALGORITHM, TOK_CONSTANTS, TOK_DATA, TOK_BEGIN, TOK_END,
    TOK_PROCEDURE, TOK_FUNCTION, TOK_INTERFACE,
    TOK_INPUT_PARAM, TOK_OUTPUT_PARAM,
    TOK_END_FUNCTION, TOK_END_PROCEDURE,
    TOK_IF, TOK_THEN, TOK_ELSE, TOK_END_IF,
    TOK_FOR, TOK_TO, TOK_STEP, TOK_REPEAT, TOK_END_FOR,
    TOK_WHILE, TOK_END_WHILE, TOK_UNTIL,
    TOK_PRINT, TOK_READ, TOK_CALCULATE,
    TOK_INTEGER_TYPE, TOK_REAL_TYPE, TOK_BOOLEAN_TYPE,
    TOK_CHAR_TYPE, TOK_STRING_TYPE, TOK_ARRAY, TOK_OF,
    TOK_ASSIGN, TOK_PLUS, TOK_MINUS, TOK_MULTIPLY, TOK_DIVIDE,
    TOK_MOD, TOK_DIV,
    TOK_EQUALS, TOK_NOT_EQUALS, TOK_LESS_THAN, TOK_GREATER_THAN,
    TOK_LESS_EQUALS, TOK_GREATER_EQUALS,
    TOK_AND, TOK_OR, TOK_NOT,
    TOK_LEFT_PAREN, TOK_RIGHT_PAREN, TOK_LEFT_BRACKET, TOK_RIGHT_BRACKET,
    TOK_COMMA, TOK_COLON, TOK_SEMICOLON, TOK_DOT, TOK_PERCENT,
    TOK_NUMBER, TOK_STRING, TOK_BOOLEAN_LITERAL, TOK_IDENTIFIER,
    TOK_EOF, TOK_ERROR
} TokenType;
...
// AST Node Types
typedef enum {
    AST_PROGRAM, AST_CONST_DECL, AST_VAR_DECL, AST_ARRAY_TYPE,
    AST_FUNC_DECL, AST_PROC_DECL, AST_PARAMETER,
    AST_ASSIGN, AST_PRINT, AST_READ, AST_IF, AST_FOR, AST_WHILE,
    AST_CALL, AST_BINARY_OP, AST_UNARY_OP, AST_LITERAL,
    AST_IDENTIFIER, AST_ARRAY_ACCESS
} ASTNodeType;
...
typedef enum {
    VAL_INT, VAL_REAL, VAL_BOOL, VAL_STRING, VAL_ARRAY, VAL_NONE
} ValueType;
...
static double to_real(RuntimeValue *val) {
    if (val->type == VAL_REAL) return val->value.real_val;
    if (val->type == VAL_INT) return (double)val->value.int_val;
    return 0.0;
}
...
static TokenType get_keyword_type(const char *str) {
    if (is_keyword(str, "ΑΛΓΟΡΙΘΜΟΣ") || is_keyword(str, "ALGORITHM")) return TOK_ALGORITHM;
    if (is_keyword(str, "ΣΤΑΘΕΡΕΣ") || is_keyword(str, "CONSTANTS")) return TOK_CONSTANTS;
    if (is_keyword(str, "ΔΕΔΟΜΕΝΑ") || is_keyword(str, "DATA")) return TOK_DATA;
    if (is_keyword(str, "ΑΡΧΗ") || is_keyword(str, "BEGIN")) return TOK_BEGIN;
    if (is_keyword(str, "ΤΕΛΟΣ") || is_keyword(str, "END")) return TOK_END;
    
    if (is_keyword(str, "ΣΥΝΑΡΤΗΣΗ") || is_keyword(str, "FUNCTION")) return TOK_FUNCTION;
    if (is_keyword(str, "ΔΙΑΔΙΚΑΣΙΑ") || is_keyword(str, "PROCEDURE")) return TOK_PROCEDURE;
    if (is_keyword(str, "ΔΙΕΠΑΦΗ") || is_keyword(str, "INTERFACE")) return TOK_INTERFACE;
    if (is_keyword(str, "ΕΙΣΟΔΟΣ")) return TOK_INPUT_PARAM;
    if (is_keyword(str, "ΕΞΟΔΟΣ")) return TOK_OUTPUT_PARAM;
    if (is_keyword(str, "ΤΕΛΟΣ-ΣΥΝΑΡΤΗΣΗΣ") || is_keyword(str, "END_FUNCTION")) return TOK_END_FUNCTION;
    if (is_keyword(str, "ΤΕΛΟΣ-ΔΙΑΔΙΚΑΣΙΑΣ") || is_keyword(str, "END_PROCEDURE")) return TOK_END_PROCEDURE;
    
    if (is_keyword(str, "ΕΑΝ") || is_keyword(str, "IF")) return TOK_IF;
    if (is_keyword(str, "ΤΟΤΕ") || is_keyword(str, "THEN")) return TOK_THEN;
    if (is_keyword(str, "ΑΛΛΙΩΣ") || is_keyword(str, "ELSE")) return TOK_ELSE;
    if (is_keyword(str, "ΕΑΝ-ΤΕΛΟΣ") || is_keyword(str, "END_IF") || is_keyword(str, "ENDIF")) return TOK_END_IF;
    
    if (is_keyword(str, "ΓΙΑ") || is_keyword(str, "FOR")) return TOK_FOR;
    if (is_keyword(str, "ΕΩΣ") || is_keyword(str, "TO")) return TOK_TO;
    if (is_keyword(str, "ΒΗΜΑ") || is_keyword(str, "ΜΕ") || is_keyword(str, "STEP")) return TOK_STEP;
    if (is_keyword(str, "ΕΠΑΝΑΛΑΒΕ") || is_keyword(str, "REPEAT")) return TOK_REPEAT;
    if (is_keyword(str, "ΓΙΑ-ΤΕΛΟΣ") || is_keyword(str, "END_FOR") || is_keyword(str, "ENDFOR")) return TOK_END_FOR;
    
    if (is_keyword(str, "ΕΝΟΣΩ") || is_keyword(str, "WHILE")) return TOK_WHILE;
    if (is_keyword(str, "ΕΝΟΣΩ-ΤΕΛΟΣ") || is_keyword(str, "END_WHILE") || is_keyword(str, "ENDWHILE")) return TOK_END_WHILE;
    if (is_keyword(str, "ΜΕΧΡΙ") || is_keyword(str, "UNTIL")) return TOK_UNTIL;
    
    if (is_keyword(str, "ΤΥΠΩΣΕ") || is_keyword(str, "PRINT")) return TOK_PRINT;
    if (is_keyword(str, "ΔΙΑΒΑΣΕ") || is_keyword(str, "READ")) return TOK_READ;
    if (is_keyword(str, "ΥΠΟΛΟΓΙΣΕ") || is_keyword(str, "CALCULATE")) return TOK_CALCULATE;
    
    if (is_keyword(str, "ΑΚΕΡΑΙΟΣ") || is_keyword(str, "INTEGER")) return TOK_INTEGER_TYPE;
    if (is_keyword(str, "ΠΡΑΓΜΑΤΙΚΟΣ") || is_keyword(str, "REAL")) return TOK_REAL_TYPE;
    if (is_keyword(str, "ΛΟΓΙΚΟΣ") || is_keyword(str, "BOOLEAN")) return TOK_BOOLEAN_TYPE;
    if (is_keyword(str, "ΧΑΡΑΚΤΗΡΑΣ") || is_keyword(str, "CHAR")) return TOK_CHAR_TYPE;
    if (is_keyword(str, "ΣΥΜΒΟΛΟΣΕΙΡΑ") || is_keyword(str, "STRING")) return TOK_STRING_TYPE;
    if (is_keyword(str, "ARRAY")) return TOK_ARRAY;
    if (is_keyword(str, "OF")) return TOK_OF;
    
    if (is_keyword(str, "ΚΑΙ") || is_keyword(str, "AND")) return TOK_AND;
    if (is_keyword(str, "Ή") || is_keyword(str, "OR")) return TOK_OR;
    if (is_keyword(str, "ΟΧΙ") || is_keyword(str, "NOT")) return TOK_NOT;
    if (is_keyword(str, "MOD")) return TOK_MOD;
    if (is_keyword(str, "DIV")) return TOK_DIV;
    
    if (is_keyword(str, "ΑΛΗΘΗΣ") || is_keyword(str, "TRUE")) return TOK_BOOLEAN_LITERAL;
    if (is_keyword(str, "ΨΕΥΔΗΣ") || is_keyword(str, "FALSE")) return TOK_BOOLEAN_LITERAL;
...
static void tokenize(const char *code) {
    ...
    while (pos < len) {
        ...
        if (pos + 1 < len) {
            ...
            if (strcmp(two_char, ":=") == 0) type = TOK_ASSIGN;
            else if (strcmp(two_char, "<>") == 0) type = TOK_NOT_EQUALS;
            else if (strcmp(two_char, "<=") == 0) type = TOK_LESS_EQUALS;
            else if (strcmp(two_char, ">=") == 0) type = TOK_GREATER_EQUALS;
            else if (strcmp(two_char, "..") == 0) type = TOK_DOT;
        ...
        switch (code[pos]) {
            ...
            case '<': type = TOK_LESS_THAN; break;
            case '>': type = TOK_GREATER_THAN; break;
            case '(': type = TOK_LEFT_PAREN; break;
            case ')': type = TOK_RIGHT_PAREN; break;
            case '[': type = TOK_LEFT_BRACKET; break;
            case ']': type = TOK_RIGHT_BRACKET; break;
...
static ASTNode *parse_print() {
    ...
    expect_token(TOK_RIGHT_PAREN);
    if (match_token(TOK_SEMICOLON)) advance_token();
...
static ASTNode *parse_call_statement() {
    ...
    if (!match_token(TOK_RIGHT_PAREN)) {
        do {
            if (match_token(TOK_PERCENT)) advance_token();
        ...
        case AST_CALL: {
            ...
            for (int i = 0; i < function->subroutine.num_params && i < expr->call.num_args; i++) {
              ...
              if (arg->type == AST_IDENTIFIER) {
                        ...
                        if (potential_array && potential_array->type == VAL_ARRAY) {  // <-- ΑΦΑΙΡΕΣΗ του && param->param.is_reference
            ...
            for (int i = 0; i < function->subroutine.num_params && i < expr->call.num_args; i++) {
                ...
                if (param->param.is_reference) {
                    ...
                    if (func_val->type == VAL_ARRAY) continue;
...
static void execute_statement(ASTNode *stmt, Environment *env) {
 ...
 case AST_ASSIGN: {
    ...
    case AST_FOR: {
            ...
            if (start_val.type != VAL_ARRAY) free_runtime_value(&start_val);
            if (end_val.type != VAL_ARRAY) free_runtime_value(&end_val);
            if (step_val.type != VAL_ARRAY) free_runtime_value(&step_val);
...
case AST_WHILE: {
    if (stmt->while_loop.is_repeat_until) {
        ...
        do {
            ...
            if (should_stop) break;  // Stop when UNTIL condition is TRUE
        } while (true);
    } else {
        ...
        while (true) {
            ...
            if (!should_continue) break;  // Stop when WHILE condition is FALSE
...
static const char* token_type_name(TokenType type) {
    switch(type) {
        case TOK_ALGORITHM: return "ALGORITHM";
        case TOK_CONSTANTS: return "CONSTANTS";
        ...
        case TOK_REPEAT: return "REPEAT/ΕΠΑΝΑΛΑΒΕ";
        case TOK_END_FOR: return "END_FOR";
        case TOK_WHILE: return "WHILE/ΕΝΟΣΩ";
        case TOK_END_WHILE: return "END_WHILE/ΕΝΟΣΩ-ΤΕΛΟΣ";
        case TOK_UNTIL: return "UNTIL/ΜΕΧΡΙ";
        case TOK_PRINT: return "PRINT/ΤΥΠΩΣΕ";
        case TOK_READ: return "READ/ΔΙΑΒΑΣΕ";
        case TOK_SEMICOLON: return "SEMICOLON ;";
        case TOK_IDENTIFIER: return "IDENTIFIER";
        case TOK_INTEGER_TYPE: return "INTEGER_TYPE";
        case TOK_LEFT_PAREN: return "LEFT_PAREN (";
        case TOK_RIGHT_PAREN: return "RIGHT_PAREN )";
        case TOK_LEFT_BRACKET: return "LEFT_BRACKET [";
        case TOK_RIGHT_BRACKET: return "RIGHT_BRACKET ]";
...
static void convert_windows1253_to_utf8(char *content, size_t size) {
    ...
    static const struct {
        ...
        {0xC1, "Α"}, {0xC2, "Β"}, {0xC3, "Γ"}, {0xC4, "Δ"}, {0xC5, "Ε"},
        {0xC6, "Ζ"}, {0xC7, "Η"}, {0xC8, "Θ"}, {0xC9, "Ι"}, {0xCA, "Κ"},
        {0xCB, "Λ"}, {0xCC, "Μ"}, {0xCD, "Ν"}, {0xCE, "Ξ"}, {0xCF, "Ο"},
        {0xD0, "Π"}, {0xD1, "Ρ"}, {0xD3, "Σ"}, {0xD4, "Τ"}, {0xD5, "Υ"},
        {0xD6, "Φ"}, {0xD7, "Χ"}, {0xD8, "Ψ"}, {0xD9, "Ω"},
        
        // Greek lowercase letters
        {0xE1, "α"}, {0xE2, "β"}, {0xE3, "γ"}, {0xE4, "δ"}, {0xE5, "ε"},
        {0xE6, "ζ"}, {0xE7, "η"}, {0xE8, "θ"}, {0xE9, "ι"}, {0xEA, "κ"},
        {0xEB, "λ"}, {0xEC, "μ"}, {0xED, "ν"}, {0xEE, "ξ"}, {0xEF, "ο"},
        {0xF0, "π"}, {0xF1, "ρ"}, {0xF2, "ς"}, {0xF3, "σ"}, {0xF4, "τ"},
        {0xF5, "υ"}, {0xF6, "φ"}, {0xF7, "χ"}, {0xF8, "ψ"}, {0xF9, "ω"},
        
        // Greek uppercase with tonos (accented)
        {0xA1, "Ά"}, {0xA2, "Έ"}, {0xA3, "Ή"}, {0xA4, "Ί"}, {0xA5, "Ό"},
        {0xA6, "Ύ"}, {0xA7, "Ώ"},
        
        // Greek lowercase with tonos (accented)
        {0xDC, "ά"}, {0xDD, "έ"}, {0xDE, "ή"}, {0xDF, "ί"}, {0xFC, "ό"},
        {0xFD, "ύ"}, {0xFE, "ώ"},
    ...
    for (size_t i = 0; i < size; i++) {
        ...
        if (c >= 0x80) {
            ...
            if (!found) new_size++;  // Keep as-is if not in map
        } else {
            new_size++;
        }
    }
...
static char *convert_windows1253_to_utf8_new(const char *content, size_t size) {
    ...
    for (size_t i = 0; i < size; i++) {
        ...
        if (c >= 0x80) {
            ...
            if (!found) new_content[out_pos++] = c;
        } else {
            new_content[out_pos++] = c;
        }
    }
...
bool is_valid_utf8(const char *str, size_t len) {
    ...
    for (size_t i = 0; i < len; ) {
        ...
        else if (bytes[i] >= 0xE1 && bytes[i] <= 0xEF && i + 2 < len && bytes[i+1] >= 0x80 && bytes[i+1] <= 0xBF && bytes[i+2] >= 0x80 && bytes[i+2] <= 0xBF) i += 3;
